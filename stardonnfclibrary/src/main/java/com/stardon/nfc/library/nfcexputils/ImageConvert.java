...
public class ImageConvert
{
    ...
    private static boolean PixelIsBlack(int color, int gray_threshold) {
    ...
    /// <param name="gray_threshold">阈值@param
    /// <returns>null表示失败，否则返回数据数组</returns>
    public static byte[] CovertImageHorizontal2Bytes(final Bitmap bitmap, final int gray_threshold)
    {
        final int widthMax = bitmap.getWidth();
        final int height = bitmap.getHeight();
        int bytesPerLine = (widthMax - 1) / 16 + 1;//2字节对齐
        bytesPerLine *= 2;
        ...
        int x = 0, y = 0;                                                       //位图的x，y坐标值；
        int index = 0;
        for (int h = 0; h < height; h++)                                        //每次判断一像素行，需要判断BmpHeight次
        {
            for (int j = 0; j < bytesPerLine; j++)                              //每行需要LengthRow字节存放数据，
            {
                for (int k = 0; k < 8; k++)                                     //每个字节存放8个点，即1点1位；
                {
                    x = (j << 3) + k;                                           //x坐标为已计算字节×8+当前字节的位k；
                    y = h;                                                                         //当前行；
                    if (x >= widthMax)                                             //如果位图当前像素点的y坐标大于实际位图宽度(位图实际宽度可能不为8的整数倍)，不对该点颜色进行判断；
                    {
                        continue;
                    }
                    else
